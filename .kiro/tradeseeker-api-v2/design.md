...
```
Client → API Gateway → Lambda → DynamoDB (golden-crosses, stock-prices, ath)
                                    ↓
                              CloudWatch Logs
```
...
│   │   └── ath.py                 # All-time high endpoint handler
│   └── db/
│       ├── __init__.py
...
### 5. All-Time High Handler (handlers/ath.py)

**Responsibility**: Handles all-time high stock queries with filtering

**Interface**:
```python
def handle_ath(query_params: dict) -> dict:
    """
    Handles GET /ath requests.
...
**Logic Flow**:
1. Validate query parameters using validator
2. Determine optimal DynamoDB query strategy:
   - If date + market: use market_code-detection_date-index GSI
   - If date only: use detection_date-index GSI
   - If days: calculate date range and use detection_date-index GSI
   - Otherwise: scan table (with warning in logs)
3. Execute DynamoDB query
4. Apply market filter if specified
5. Apply limit if specified (take most recent N records)
6. Format and return results

### 6. Validator (validators.py)
...
def validate_ath_params(params: dict) -> tuple[dict, list[str]]:
    """
    Validates all-time high query parameters.
...
class DynamoDBClient:
    ...
    def query_ath_by_date(
        self, 
        date: str, 
        market_code: str = None
    ) -> list[dict]:
        """
        Query all-time highs by date using GSI.
    ...
    def query_ath_by_date_range(
        ...
        """
        Query all-time highs within date range using GSI.
    ...
    def scan_ath(self, market_code: str = None) -> list[dict]:
        """
        Scan all-time highs table (use sparingly).
...
**Source**: ts-batch-v2-dev-ath DynamoDB table

**Schema**:
```python
{
    "symbol": str,              # PK: Stock symbol with market code (e.g., "AAPL.US")
    "detection_date": str,      # SK: Date when ATH was detected (YYYY-MM-DD)
    "ath_price": float,         # The all-time high price that was reached
    "ath_percentage_gain": float, # Percentage gain from lowest price to new ATH
    "market_code": str,         # Market code (e.g., "US", "BK", "CC")
    "detected_at": str,         # ISO timestamp when detection was processed
    "ttl": float                # Unix timestamp for TTL (30 days from detection)
}
```

**Indexes**:
- Primary: symbol (PK), detection_date (SK)
- GSI: detection_date-index (detection_date as PK)
- GSI: market_code-detection_date-index (market_code as PK, detection_date as SK)
...
### Property 9: ATH Date Filtering

*For any* request to /ath with date parameter, all returned records should have detection_date equal to the specified date. For requests with days parameter, all returned records should have detection_date within the calculated date range.

**Validates: ATH endpoint date filtering requirements**
