...
5. WHEN querying stock prices by symbol, THE API SHALL use the GetItem operation with the symbol partition key
6. THE API SHALL implement read-only access to DynamoDB tables with no write operations
7. WHEN a DynamoDB operation fails, THE API SHALL return a 500 status code with an error message

### Requirement 5: AWS Lambda Deployment
