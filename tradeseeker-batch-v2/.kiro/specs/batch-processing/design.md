...
- ATH detection: Automated monitoring for all-time highs within 30 days of golden cross signals integrated into downloader lambda
...
- **Purpose**: Fetches stock price data, stores in S3, calculates exponential moving averages, saves to DynamoDB, and detects all-time highs
- **Responsibilities**:
  ...
  - Detects all-time high events and stores in ath-detections table
...
- **Interfaces**:
  ...
  - Output: S3 objects, DynamoDB records (stock-prices and ath-detections), CloudWatch logs/metrics
...
### 2.7 ATH Detections DynamoDB Table
- **Purpose**: Storage for all-time high detection events that occur within 30 days of golden cross signals
- **Table Name**: `ath-detections`
- **Primary Key**:
  - Partition Key: `symbol` (String) - e.g., "AAPL.US", "PTT.BK"
  - Sort Key: `detection_date` (String) - Date when ATH was detected (YYYY-MM-DD)
- **Attributes**:
  - `symbol`: Stock symbol with market code
  - `detection_date`: Date when ATH was detected
  - `ath_price`: The all-time high price that was reached
  - `ath_percentage_gain`: Percentage gain from lowest price to new ATH
  - `market_code`: Market code (e.g., "US", "BK", "CC")
  - `detected_at`: Timestamp when detection was processed
  - `ttl`: Time-to-live (auto-deletes after 30 days)
- **GSI Indexes**:
  - `detection_date-index`: Query all ATH detections by date
  - `market_code-detection_date-index`: Query ATH detections by market and date
- **Configuration**:
  - Billing mode: On-demand (pay per request)
  - Point-in-time recovery: Enabled
  - Encryption: AWS managed key
  - TTL: Enabled on `ttl` attribute (30-day retention)
...
### 3.4 ATH Detection Record Schema
```typescript
interface ATHDetectionRecord {
...
Example ATH detection record:
```json
{
  "symbol": "AAPL.US",
  "detection_date": "2024-02-20",
  "ath_price": 195.50,
  "ath_percentage_gain": 125.75,
  "market_code": "US",
  "detected_at": "2024-02-20T09:00:00Z",
  "ttl": 1716192000
}
```
...
def save_to_dynamodb(task: Dict, price_data: List[Dict]):
    ...
    check_and_save_ath_detection(symbol_with_market, task['marketCode'], sorted_data)
...
def check_and_save_ath_detection(symbol: str, market_code: str, price_data: List[Dict]):
    """Check if symbol has recent golden cross and detect ATH within 30 days"""
    try:
        # Check if symbol has golden cross in past 30 days
        if not has_recent_golden_cross(symbol):
            return
        ...
        # Calculate historical maximum and minimum
        historical_prices = [
        ...
        print(f"Error in ATH detection for {symbol}: {str(e)}")

def has_recent_golden_cross(symbol: str) -> bool:
    """Check if symbol has golden cross signal in past 30 days"""
    try:
        dynamodb = boto3.resource('dynamodb')
        table = dynamodb.Table('golden-crosses')
        ...
        print(f"Error checking golden cross for {symbol}: {str(e)}")
        return False

def save_ath_detection(detection: Dict):
    """Save ATH detection record to DynamoDB"""
    try:
        dynamodb = boto3.resource('dynamodb')
        table = dynamodb.Table('ath-detections')
        ...
        if 'Item' not in existing:
            ...
            print(f"ATH detection saved: {detection['symbol']} - {detection['ath_price']}")
        else:
            print(f"Duplicate ATH detection skipped: {detection['symbol']} on {detection['detection_date']}")
            
    except Exception as e:
        print(f"Error saving ATH detection for {detection['symbol']}: {str(e)}")
```
...
### 5.6 ATH Detection Window Validation
- **Description**: ATH detections only occur within 30 days of golden cross signals
- **Implementation**: Date validation logic in get_recent_golden_crosses ensures only symbols with golden crosses in past 30 days are checked
- **Testing**: Unit tests verify window calculation with edge cases (month boundaries, leap years)
- **Validates**: ATH Detection Requirements

### 5.7 ATH Price Accuracy
- **Description**: ATH detection correctly identifies when current price exceeds historical maximum
- **Implementation**: Compare current closing price against max of all historical closing prices before detection date
- **Testing**: Property tests verify ATH detection with generated price sequences
- **Validates**: ATH Detection Requirements

### 5.8 Duplicate ATH Prevention
- **Description**: Multiple runs of ATH detection for same symbol/date do not create duplicate records
- **Implementation**: Primary key constraint (symbol, detection_date) prevents duplicates; check before insert
- **Testing**: Integration tests verify duplicate detection and prevention
- **Validates**: ATH Detection Requirements
...
- ATH detection: Date window validation, ATH price calculation, duplicate detection logic (integrated in downloader)
- Mock external dependencies (EODHD API, AWS services) using `moto` and `pytest` fixtures

### 6.2 Integration Tests
- End-to-end: EventBridge → Task Generator → SQS → Downloader → S3 + DynamoDB (stock-prices and ath-detections) (using LocalStack or moto)
...
- ATH detection flow: Integrated with downloader lambda → DynamoDB (ath-detections table)
- Test with sample data from multiple markets (US, BK, CC)

### 6.3 Property-Based Tests
- ATH detection accuracy: Generate random price sequences, verify ATH detection correctness
- Window validation: Generate random date combinations, verify 30-day window enforcement
- Duplicate prevention: Test multiple detection runs for same symbol/date combinations
...
- SQS (Standard queues)
- S3
- DynamoDB (stock-prices, golden-crosses, ath-detections tables)
...
- `ATH-Detection/ATHDetections`: Number of ATH events detected (emitted by downloader)
...
- `ath_detection`: ATH detection result (if applicable)
